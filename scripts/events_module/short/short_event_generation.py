...
def generate_event_objects(event_triggered, biome, frequency) -> list:
    ...
    def get(event, key, default):
        return event[key] if key in event else default
    
    def get_all(event, pairs):
        return { key: get(event, key, default) for key, default in pairs }
    ...
    for event in events_dict:
        ...
        if "r_c" in event and not args["r_c"]:
            args["r_c"] = {"age": ["any"]}

        event_list.append(ShortEvent(**args))
